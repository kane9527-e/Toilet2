...
namespace UnityGameFramework.Editor
{
    ...
    {
        private static readonly string[] ResourceModeNames = { "Package", "Updatable", "Updatable While Playing" };
        ...
        private readonly HelperInfo<LoadResourceAgentHelperBase> m_LoadResourceAgentHelperInfo =
            new HelperInfo<LoadResourceAgentHelperBase>("LoadResourceAgent");
        ...
        private readonly HelperInfo<ResourceHelperBase> m_ResourceHelperInfo =
            new HelperInfo<ResourceHelperBase>("Resource");
        ...
        private void OnEnable()
        {
            ...
            m_GenerateReadWriteVersionListLength =
                serializedObject.FindProperty("m_GenerateReadWriteVersionListLength");
            ...
            m_EditorResourceModeFieldInfo = target.GetType()
        ...
        public override void OnInspectorGUI()
        {
            ...
            var isEditorResourceMode = (bool)m_EditorResourceModeFieldInfo.GetValue(target);
            ...
            EditorGUI.BeginDisabledGroup(EditorApplication.isPlayingOrWillChangePlaymode);
            {
                ...
                {
                    var selectedIndex = EditorGUILayout.Popup("Resource Mode", m_ResourceModeIndex, ResourceModeNames);
                ...
                m_ReadWritePathType.enumValueIndex =
                    (int)(ReadWritePathType)EditorGUILayout.EnumPopup("Read-Write Path Type", t.ReadWritePathType);
            }
            EditorGUI.EndDisabledGroup();

            var minUnloadUnusedAssetsInterval = EditorGUILayout.Slider("Min Unload Unused Assets Interval",
            ...
            var maxUnloadUnusedAssetsInterval = EditorGUILayout.Slider("Max Unload Unused Assets Interval",
            ...
            {
                var assetAutoReleaseInterval = EditorGUILayout.DelayedFloatField("Asset Auto Release Interval",
                ...
                var assetCapacity = EditorGUILayout.DelayedIntField("Asset Capacity", m_AssetCapacity.intValue);
                ...
                var assetExpireTime =
                    EditorGUILayout.DelayedFloatField("Asset Expire Time", m_AssetExpireTime.floatValue);
                ...
                var assetPriority = EditorGUILayout.DelayedIntField("Asset Priority", m_AssetPriority.intValue);
                ...
                var resourceAutoReleaseInterval = EditorGUILayout.DelayedFloatField("Resource Auto Release Interval",
                    m_ResourceAutoReleaseInterval.floatValue);
                ...
                var resourceCapacity =
                    EditorGUILayout.DelayedIntField("Resource Capacity", m_ResourceCapacity.intValue);
                ...
                var resourceExpireTime =
                    EditorGUILayout.DelayedFloatField("Resource Expire Time", m_ResourceExpireTime.floatValue);
                ...
                var resourcePriority =
                    EditorGUILayout.DelayedIntField("Resource Priority", m_ResourcePriority.intValue);
                ...
                {
                    var updatePrefixUri =
                        EditorGUILayout.DelayedTextField("Update Prefix Uri", m_UpdatePrefixUri.stringValue);
                    ...
                    var generateReadWriteVersionListLength = EditorGUILayout.DelayedIntField(
                        "Generate Read-Write Version List Length", m_GenerateReadWriteVersionListLength.intValue);
                    ...
                    var updateRetryCount =
                        EditorGUILayout.DelayedIntField("Update Retry Count", m_UpdateRetryCount.intValue);
            ...
            EditorGUI.BeginDisabledGroup(EditorApplication.isPlayingOrWillChangePlaymode);
            {
                ...
                m_LoadResourceAgentHelperCount.intValue = EditorGUILayout.IntSlider("Load Resource Agent Helper Count",
                    m_LoadResourceAgentHelperCount.intValue, 1, 128);
            }
            EditorGUI.EndDisabledGroup();
            ...
            if (EditorApplication.isPlaying && IsPrefabInHierarchy(t.gameObject))
            {
                EditorGUILayout.LabelField("Unload Unused Assets",
                    Utility.Text.Format("{0} / {1}", t.LastUnloadUnusedAssetsOperationElapseSeconds.ToString("F2"),
                        t.MaxUnloadUnusedAssetsInterval.ToString("F2")));
                ...
                EditorGUILayout.LabelField("Applicable Game Version",
                    isEditorResourceMode ? "N/A" : t.ApplicableGameVersion ?? "<Unknwon>");
                EditorGUILayout.LabelField("Internal Resource Version",
                    isEditorResourceMode ? "N/A" : t.InternalResourceVersion.ToString());
                ...
                EditorGUILayout.LabelField("Resource Group Count",
                    isEditorResourceMode ? "N/A" : t.ResourceGroupCount.ToString());
                if (m_ResourceModeIndex > 0)
                {
                    EditorGUILayout.LabelField("Applying Resource Pack Path",
                        isEditorResourceMode ? "N/A" : t.ApplyingResourcePackPath ?? "<Unknwon>");
                    EditorGUILayout.LabelField("Apply Waiting Count",
                        isEditorResourceMode ? "N/A" : t.ApplyWaitingCount.ToString());
                    EditorGUILayout.LabelField("Updating Resource Group",
                        isEditorResourceMode ? "N/A" :
                        t.UpdatingResourceGroup != null ? t.UpdatingResourceGroup.Name : "<Unknwon>");
                    EditorGUILayout.LabelField("Update Waiting Count",
                        isEditorResourceMode ? "N/A" : t.UpdateWaitingCount.ToString());
                    EditorGUILayout.LabelField("Update Waiting While Playing Count",
                        isEditorResourceMode ? "N/A" : t.UpdateWaitingWhilePlayingCount.ToString());
                    EditorGUILayout.LabelField("Update Candidate Count",
                        isEditorResourceMode ? "N/A" : t.UpdateCandidateCount.ToString());
                }

                EditorGUILayout.LabelField("Load Total Agent Count",
                    isEditorResourceMode ? "N/A" : t.LoadTotalAgentCount.ToString());
                EditorGUILayout.LabelField("Load Free Agent Count",
                    isEditorResourceMode ? "N/A" : t.LoadFreeAgentCount.ToString());
                EditorGUILayout.LabelField("Load Working Agent Count",
                    isEditorResourceMode ? "N/A" : t.LoadWorkingAgentCount.ToString());
                EditorGUILayout.LabelField("Load Waiting Task Count",
                    isEditorResourceMode ? "N/A" : t.LoadWaitingTaskCount.ToString());
                if (!isEditorResourceMode)
                {
                    EditorGUILayout.BeginVertical("box");
                    {
                        ...
                        {
                            foreach (var loadAssetInfo in loadAssetInfos) DrawLoadAssetInfo(loadAssetInfo);
                            ...
                            if (GUILayout.Button("Export CSV Data"))
                            {
                                ...
                                if (!string.IsNullOrEmpty(exportFileName))
                                    try
                                    {
                                        ...
                                        foreach (var loadAssetInfo in loadAssetInfos)
                                            data[index++] = Utility.Text.Format("{0},{1},{2},{3}",
                                                loadAssetInfo.Description, loadAssetInfo.SerialId.ToString(),
                                                loadAssetInfo.Priority.ToString(), loadAssetInfo.Status.ToString());

                                        File.WriteAllLines(exportFileName, data, Encoding.UTF8);
                                        Debug.Log(Utility.Text.Format(
                                            "Export load asset task CSV data to '{0}' success.", exportFileName));
                                    }
        ...
        private void DrawLoadAssetInfo(TaskInfo loadAssetInfo)
        {
            EditorGUILayout.LabelField(loadAssetInfo.Description,
                Utility.Text.Format("[SerialId]{0} [Priority]{1} [Status]{2}", loadAssetInfo.SerialId.ToString(),
                    loadAssetInfo.Priority.ToString(), loadAssetInfo.Status.ToString()));
        }
