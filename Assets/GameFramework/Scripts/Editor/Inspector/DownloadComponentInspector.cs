...
namespace UnityGameFramework.Editor
{
    ...
    internal sealed class DownloadComponentInspector : GameFrameworkInspector
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var t = (DownloadComponent)target;
            ...
            var flushSize = EditorGUILayout.DelayedIntField("Flush Size", m_FlushSize.intValue);
            ...
            if (EditorApplication.isPlaying && IsPrefabInHierarchy(t.gameObject))
            {
                ...
                EditorGUILayout.BeginVertical("box");
                {
                    ...
                    {
                        foreach (var downloadInfo in downloadInfos) DrawDownloadInfo(downloadInfo);
                                ...
                                try
                                {
                                    ...
                                    foreach (var downloadInfo in downloadInfos)
                                        data[index++] = Utility.Text.Format("{0},{1},{2},{3},{4}",
                                            downloadInfo.Description, downloadInfo.SerialId.ToString(),
                                            downloadInfo.Tag ?? string.Empty, downloadInfo.Priority.ToString(),
        ...
        private void DrawDownloadInfo(TaskInfo downloadInfo)
        {
            EditorGUILayout.LabelField(downloadInfo.Description,
                Utility.Text.Format("[SerialId]{0} [Tag]{1} [Priority]{2} [Status]{3}",
                    downloadInfo.SerialId.ToString(), downloadInfo.Tag ?? "<None>", downloadInfo.Priority.ToString(),
                    downloadInfo.Status.ToString()));
        }
