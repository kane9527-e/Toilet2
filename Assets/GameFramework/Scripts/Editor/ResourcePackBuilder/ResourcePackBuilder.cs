...
namespace UnityGameFramework.Editor.ResourceTools
{
    ...
    {
        private static readonly string[] PlatformForDisplay =
            { "Windows", "Windows x64", "macOS", "Linux", "iOS", "Android", "Windows Store", "WebGL" };

        private static readonly int[] LengthLimit = { 0, 128, 256, 512, 1024, 2048, 4096 };

        private static readonly string[] LengthLimitForDisplay =
        ...
        private void OnEnable()
        {
            ...
            var compressionHelperTypeNames = m_Controller.GetCompressionHelperTypeNames();
            for (var i = 0; i < compressionHelperTypeNames.Length; i++)
            ...
            EditorGUILayout.BeginVertical(GUILayout.Width(position.width), GUILayout.Height(position.height));
            {
                ...
                EditorGUILayout.BeginVertical("box");
                {
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        var directory = EditorGUILayout.TextField(m_Controller.WorkingDirectory);
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        var platformIndex = EditorGUILayout.Popup(m_PlatformIndex, PlatformForDisplay);
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        var names = m_Controller.GetCompressionHelperTypeNames();
                        var selectedIndex = EditorGUILayout.Popup(m_CompressionHelperTypeNameIndex, names);
                    ...
                    else
                    {
                        ...
                        EditorGUILayout.BeginHorizontal();
                        {
                            ...
                            {
                                var lengthLimitIndex = EditorGUILayout.Popup(m_LengthLimitIndex, LengthLimitForDisplay);
                                ...
                                if (m_LengthLimitIndex >= LengthLimit.Length)
                                {
                                    EditorGUILayout.BeginHorizontal();
                                    {
                                        ...
                                        if (m_Controller.LengthLimit < 0) m_Controller.LengthLimit = 0;
                        ...
                        EditorGUILayout.BeginHorizontal();
                        {
                            ...
                            var value = EditorGUILayout.Popup(m_TargetVersionIndex, m_VersionNamesForTargetDisplay);
                        ...
                        EditorGUILayout.BeginHorizontal();
                        {
                            ...
                            EditorGUILayout.BeginVertical();
                            {
                                EditorGUILayout.BeginHorizontal();
                                {
                                    EditorGUILayout.LabelField(m_SourceVersionCount +
                                    ...
                                    if (GUILayout.Button("Select All Except <None>", GUILayout.Width(180f)))
                                    {
                                        ...
                                        for (var i = 1; i < m_SourceVersionIndexes.Length; i++)
                                    ...
                                    {
                                        for (var i = 0; i < m_SourceVersionIndexes.Length; i++)
                                            m_SourceVersionIndexes[i] = true;
                                ...
                                EditorGUILayout.BeginHorizontal();
                                {
                                    var count = m_VersionNamesForSourceDisplay.Length;
                                    var column = 5;
                                    var row = (count - 1) / column + 1;
                                    for (var i = 0; i < column && i < count; i++)
                                    {
                                        EditorGUILayout.BeginVertical();
                                        {
                                            for (var j = 0; j < row; j++)
                                            {
                                                ...
                                                if (index < count)
                                                {
                                                    var isTarget = index - 1 == m_TargetVersionIndex;
                                                    EditorGUI.BeginDisabledGroup(isTarget);
                                                    {
                                                        var selected = GUILayout.Toggle(m_SourceVersionIndexes[index],
                                                            isTarget
                ...
                EditorGUILayout.BeginHorizontal();
                {
                                                 ...
                                                 !m_Controller.IsValidWorkingDirectory || m_SourceVersionCount <= 0);
                    {
                        ...
                        {
                            var sourceVersions = new string[m_SourceVersionCount];
                            var count = 0;
                            for (var i = 0; i < m_SourceVersionIndexes.Length; i++)
        ...
        {
            var window = GetWindow<ResourcePackBuilder>("Resource Pack Builder", true);
            window.minSize = new Vector2(800f, 400f);
        }

        private void BrowseWorkingDirectory()
        {
            var directory = EditorUtility.OpenFolderPanel("Select Working Directory", m_Controller.WorkingDirectory,
                string.Empty);
            if (!string.IsNullOrEmpty(directory)) m_Controller.WorkingDirectory = directory;
        }
        ...
        private void RefreshVersionNames()
        {
            ...
            for (var i = 0; i < m_VersionNames.Length; i++)
            {
                var versionNameForDisplay = GetVersionNameForDisplay(m_VersionNames[i]);
        ...
        private void RefreshSourceVersionCount()
        {
            ...
            m_SourceVersionCount = 0;
            if (m_SourceVersionIndexes == null) return;
        ...
        {
            if (string.IsNullOrEmpty(versionName)) return "<None>";

            var splitedVersionNames = versionName.Split('_');
            if (splitedVersionNames.Length < 2) return null;

            var text = splitedVersionNames[0];
            for (var i = 1; i < splitedVersionNames.Length - 1; i++) text += "." + splitedVersionNames[i];
        ...
        private void OnBuildResourcePacksStarted(int count)
        {
            ...
            EditorUtility.DisplayProgressBar("Build Resource Packs",
                Utility.Text.Format("Build resource packs, {0} items to be built.", count.ToString()), 0f);
        }

        private void OnBuildResourcePacksCompleted(int successCount, int count)
        {
            var failureCount = count - successCount;
            var str = Utility.Text.Format("Build resource packs completed, '{0}' items, '{1}' success, '{2}' failure.",
                count.ToString(), successCount.ToString(), failureCount.ToString());
            if (failureCount > 0)
        ...
        private void OnBuildResourcePackSuccess(int index, int count, string sourceVersion, string targetVersion)
        {
            ...
            EditorUtility.DisplayProgressBar("Build Resource Packs",
                Utility.Text.Format("Build resource packs, {0}/{1} completed.", (index + 1).ToString(),
                    count.ToString()), (float)index / count);
        }
