...
namespace VisualGraphRuntime
{
    ...
    public abstract class VisualGraph : ScriptableObject
    {
	    ...
	    public virtual VisualGraph Clone()
        {
            ...
            {
                var newNode = Instantiate(Nodes[i]);
                newNode.graph = clone;
	    ...
	    public void InitializeGraph()
        {
            foreach (var node in Nodes)
            {
                ...
                foreach (var connection in port.Connections)
                    ...
                    {
                        var otherNode = FindNodeByGuid(connection.node_guid);
                        if (otherNode != null)
                        {
                            var otherPort = otherNode.FindPortByGuid(connection.port_guid);
                            if (otherPort != null)
                            {
                                var otherConnection = otherPort.FindConnectionByNodeGuid(node.guid);
        ...
        {
            var graphNode = CreateInstance(nodeType) as VisualGraphNode;
            graphNode.graph = this;
            graphNode.Init();
            Nodes.Add(graphNode);

            var dynamicsAttrib = graphNode.GetType().GetCustomAttribute<NodePortAggregateAttribute>();
            ...
            {
                var graphPort = graphNode.AddPort("Input", VisualGraphPort.PortDirection.Input);
                graphPort.CanBeRemoved = false;
            }

            if (dynamicsAttrib.OutputPortAggregate == NodePortAggregateAttribute.PortAggregate.Single)
            {
                var graphPort = graphNode.AddPort("Exit", VisualGraphPort.PortDirection.Output);
                graphPort.CanBeRemoved = false;
            }
	    ...
	    public virtual void RemoveNode(VisualGraphNode graphNode)
        {
            ...
            if (Application.isPlaying) Destroy(graphNode);
        }
	    ...
	    public bool GetPropertyValue<T>(string propertyName, ref T value)
        {
            for (var i = 0; i < BlackboardProperties.Count; i++)
                ...
                {
                    var prop = (AbstractBlackboardProperty<T>)BlackboardProperties[i];
                    if (prop != null)
                    {
                        value = prop.Value;
                        return true;
