...
namespace UnityGameFramework.Editor.ResourceTools
{
    public sealed partial class ResourceBuilderController
    {
        ...
        public ResourceBuilderController()
        {
            ...
            {
                if (OnAnalyzingAsset != null) OnAnalyzingAsset(index, count);
            };
        ...
        public string WorkingPath
        {
            get
            {
                if (!IsValidOutputDirectory) return string.Empty;
        ...
        public bool Save()
        {
            try
            {
                ...
                var xmlBuilder = xmlDocument.CreateElement("ResourceBuilder");
        ...
        public bool RefreshBuildEventHandler()
        {
            ...
            {
                var buildEventHandlerType = Utility.Assembly.GetType(BuildEventHandlerTypeName);
                if (buildEventHandlerType != null)
                {
                    var buildEventHandler = (IBuildEventHandler)Activator.CreateInstance(buildEventHandlerType);
        ...
        {
            if (!IsValidOutputDirectory) return false;

            if (Directory.Exists(OutputPackagePath)) Directory.Delete(OutputPackagePath, true);
            ...
            if (Directory.Exists(OutputPackedPath)) Directory.Delete(OutputPackedPath, true);

            Directory.CreateDirectory(OutputPackedPath);

            if (Directory.Exists(BuildReportPath)) Directory.Delete(BuildReportPath, true);

            Directory.CreateDirectory(BuildReportPath);

            var buildAssetBundleOptions = GetBuildAssetBundleOptions();
            ...
            try
            {
                m_BuildReport.LogInfo("Build Start Time: {0}",
                ...
                var watchResult = m_BuildEventHandler == null || !m_BuildEventHandler.ContinueOnFailure;
                var isSuccess = false;
            ...
            catch (Exception exception)
            {
                ...
                m_BuildReport.SaveReport();
                if (BuildResourceError != null) BuildResourceError(errorMessage);
        ...
        {
            if (!IsPlatformSelected(platform)) return true;

            var platformName = platform.ToString();
            m_BuildReport.LogInfo("Start build resources for '{0}'...", platformName);

            var workingPath = Utility.Text.Format("{0}{1}/", WorkingPath, platformName);
            m_BuildReport.LogInfo("Working path is '{0}'.", workingPath);

            var outputPackagePath = Utility.Text.Format("{0}{1}/", OutputPackagePath, platformName);
            ...
            var outputFullPath = Utility.Text.Format("{0}{1}/", OutputFullPath, platformName);
            ...
            var outputPackedPath = Utility.Text.Format("{0}{1}/", OutputPackedPath, platformName);
            ...
            foreach (var assetBundleResourceData in assetBundleResourceDatas)
                validNames.Add(GetResourceFullName(assetBundleResourceData.Name, assetBundleResourceData.Variant)
                    .ToLowerInvariant());

            if (Directory.Exists(workingPath))
            {
                var workingUri = new Uri(workingPath, UriKind.Absolute);
                var fileNames = Directory.GetFiles(workingPath, "*", SearchOption.AllDirectories);
                foreach (var fileName in fileNames)
                {
                    if (fileName.EndsWith(".manifest", StringComparison.Ordinal)) continue;

                    var relativeName = workingUri.MakeRelativeUri(new Uri(fileName, UriKind.Absolute)).ToString();
                    if (!validNames.Contains(relativeName)) File.Delete(fileName);
                }

                var manifestNames = Directory.GetFiles(workingPath, "*.manifest", SearchOption.AllDirectories);
            ...
            var assetBundleManifest = BuildPipeline.BuildAssetBundles(workingPath, assetBundleBuildDatas,
            ...
            for (var i = 0; i < assetBundleResourceDatas.Length; i++)
            {
                var fullName =
                ...
                if (!ProcessAssetBundle(platform, workingPath, outputPackagePath, outputFullPath, outputPackedPath,
                    ...
                    assetBundleResourceDatas[i].Variant, assetBundleResourceDatas[i].FileSystem)) return false;

                m_BuildReport.LogInfo("Process asset bundle '{0}' for '{1}' complete.", fullName, platformName);
            }

            // Process Binaries
            for (var i = 0; i < binaryResourceDatas.Length; i++)
            {
                var fullName = GetResourceFullName(binaryResourceDatas[i].Name, binaryResourceDatas[i].Variant);
                ...
                if (!ProcessBinary(platform, workingPath, outputPackagePath, outputFullPath, outputPackedPath,
                    ...
                    binaryResourceDatas[i].FileSystem)) return false;
            ...
            if (OutputFullSelected)
            {
                var versionListData = ProcessUpdatableVersionList(outputFullPath, platform);
                m_BuildReport.LogInfo(
                    ...
                    versionListData.CompressedLength.ToString(), versionListData.CompressedHashCode);
                if (m_BuildEventHandler != null)
                {
            ...
            if (ProcessResourceComplete != null) ProcessResourceComplete(platform);
        ...
        {
            var fullName = GetResourceFullName(name, variant);
            var resourceData = m_ResourceDatas[fullName];
            var workingName = Utility.Path.GetRegularPath(Path.Combine(workingPath, fullName.ToLowerInvariant()));

            var bytes = File.ReadAllBytes(workingName);
            var length = bytes.Length;
            var hashCode = Utility.Verifier.GetCrc32(bytes);
            var compressedLength = length;
            var compressedHashCode = hashCode;

            var hashBytes = Utility.Converter.GetBytes(hashCode);
            ...
            string variant, string fileSystem)
        {
            var fullName = GetResourceFullName(name, variant);
            var resourceData = m_ResourceDatas[fullName];
            var assetName = resourceData.GetAssetNames()[0];
        ...
        {
            var originalAssets = m_ResourceCollection.GetAssets();
            var assets = new PackageVersionList.Asset[originalAssets.Length];
            for (var i = 0; i < assets.Length; i++)
            {
                var originalAsset = originalAssets[i];
            ...
            var index = 0;
            var resources = new PackageVersionList.Resource[m_ResourceCollection.ResourceCount];
            foreach (var resourceData in resourceDatas)
            {
                var resourceCode = resourceData.GetCode(platform);
            ...
            var fileSystemNames = GetFileSystemNames(resourceDatas);
            var fileSystems = new PackageVersionList.FileSystem[fileSystemNames.Length];
            ...
            var resourceGroupNames = GetResourceGroupNames(resourceDatas);
            var resourceGroups = new PackageVersionList.ResourceGroup[resourceGroupNames.Length];
            ...
            var versionList = new PackageVersionList(ApplicableGameVersion, InternalResourceVersion, assets, resources,
            ...
            var packageVersionListPath =
                Utility.Path.GetRegularPath(Path.Combine(outputPackagePath, RemoteVersionListFileName));
            using (var fileStream = new FileStream(packageVersionListPath, FileMode.Create, FileAccess.Write))
            {
        ...
        {
            var originalAssets = m_ResourceCollection.GetAssets();
            var assets = new UpdatableVersionList.Asset[originalAssets.Length];
            for (var i = 0; i < assets.Length; i++)
            {
                var originalAsset = originalAssets[i];
            ...
            var index = 0;
            var resources = new UpdatableVersionList.Resource[m_ResourceCollection.ResourceCount];
            foreach (var resourceData in resourceDatas)
            {
                var resourceCode = resourceData.GetCode(platform);
            ...
            var fileSystemNames = GetFileSystemNames(resourceDatas);
            var fileSystems = new UpdatableVersionList.FileSystem[fileSystemNames.Length];
            for (var i = 0; i < fileSystems.Length; i++)
                fileSystems[i] = new UpdatableVersionList.FileSystem(fileSystemNames[i],
                    GetResourceIndexesFromFileSystem(resourceDatas, fileSystemNames[i]));

            var resourceGroupNames = GetResourceGroupNames(resourceDatas);
            var resourceGroups = new UpdatableVersionList.ResourceGroup[resourceGroupNames.Length];
            for (var i = 0; i < resourceGroups.Length; i++)
                resourceGroups[i] = new UpdatableVersionList.ResourceGroup(resourceGroupNames[i],
                    GetResourceIndexesFromResourceGroup(resourceDatas, resourceGroupNames[i]));

            var versionList = new UpdatableVersionList(ApplicableGameVersion, InternalResourceVersion, assets,
                resources, fileSystems, resourceGroups);
            ...
            var updatableVersionListPath =
                Utility.Path.GetRegularPath(Path.Combine(outputFullPath, RemoteVersionListFileName));
            using (var fileStream = new FileStream(updatableVersionListPath, FileMode.Create, FileAccess.Write))
            {
                if (!serializer.Serialize(fileStream, versionList))
                    throw new GameFrameworkException("Serialize updatable version list failure.");
            }

            var bytes = File.ReadAllBytes(updatableVersionListPath);
            var length = bytes.Length;
            var hashCode = Utility.Verifier.GetCrc32(bytes);
        ...
        {
            var packedResourceDatas = GetPackedResourceDatas();

            var resources = new LocalVersionList.Resource[packedResourceDatas.Length];
            for (var i = 0; i < resources.Length; i++)
            {
                var resourceData = packedResourceDatas[i];
                var resourceCode = resourceData.GetCode(platform);
            ...
            var packedFileSystemNames = GetFileSystemNames(packedResourceDatas);
            var fileSystems = new LocalVersionList.FileSystem[packedFileSystemNames.Length];
            for (var i = 0; i < fileSystems.Length; i++)
            ...
            var readOnlyVersionListPath =
                Utility.Path.GetRegularPath(Path.Combine(outputPackedPath, LocalVersionListFileName));
            using (var fileStream = new FileStream(readOnlyVersionListPath, FileMode.Create, FileAccess.Write))
            {
                if (!serializer.Serialize(fileStream, versionList))
        ...
        {
            var dependencyAssetIndexes = new List<int>();
            var assets = m_ResourceCollection.GetAssets();
            var dependencyData = m_ResourceAnalyzerController.GetDependencyData(assetName);
            foreach (var dependencyAsset in dependencyData.GetDependencyAssets())
                for (var i = 0; i < assets.Length; i++)
        ...
        {
            var assets = m_ResourceCollection.GetAssets();
            var assetGuids = resourceData.GetAssetGuids();
            var assetIndexes = new int[assetGuids.Length];
            for (var i = 0; i < assetGuids.Length; i++)
            {
                assetIndexes[i] = Array.BinarySearch(assets, m_ResourceCollection.GetAsset(assetGuids[i]));
                if (assetIndexes[i] < 0) throw new GameFrameworkException("Asset is invalid.");
            }
        ...
        {
            var packedResourceDatas = new List<ResourceData>();
            foreach (var resourceData in m_ResourceDatas.Values)
            {
                if (!resourceData.Packed) continue;
        ...
        {
            var fileSystemNames = new HashSet<string>();
            foreach (var resourceData in resourceDatas)
            {
                if (resourceData.FileSystem == null) continue;
        ...
        {
            var index = 0;
            var resourceIndexes = new List<int>();
            foreach (var resourceData in resourceDatas)
            {
                if (resourceData.FileSystem == fileSystemName) resourceIndexes.Add(index);
        ...
        {
            var resourceGroupNames = new HashSet<string>();
            foreach (var resourceData in resourceDatas)
            foreach (var resourceGroup in resourceData.GetResourceGroups())
            ...
            Dictionary<string, IFileSystem> outputFileSystem)
        {
            ...
            {
                var fileCount = GetResourceIndexesFromFileSystem(resourceDatas, fileSystemName).Length;
                var fullPath = Utility.Path.GetRegularPath(Path.Combine(outputPath,
            ...
            int compressedHashCode)
        {
            ...
            if (OutputPackageSelected)
            {
                if (string.IsNullOrEmpty(fileSystem))
                {
                    ...
                    var packageDirectoryName = Path.GetDirectoryName(packagePath);
                    if (!Directory.Exists(packageDirectoryName)) Directory.CreateDirectory(packageDirectoryName);

                    File.WriteAllBytes(packagePath, bytes);
                }
                else
                {
                    if (!m_OutputPackageFileSystems[fileSystem].WriteFile(fullNameWithExtension, bytes)) return false;
                }
            }
            ...
            if (OutputPackedSelected && resourceData.Packed)
            {
                if (string.IsNullOrEmpty(fileSystem))
                {
                    var packedPath = Utility.Path.GetRegularPath(Path.Combine(outputPackedPath, fullNameWithExtension));
                    var packedDirectoryName = Path.GetDirectoryName(packedPath);
                    if (!Directory.Exists(packedDirectoryName)) Directory.CreateDirectory(packedDirectoryName);

                    File.WriteAllBytes(packedPath, bytes);
                }
                else
                {
                    if (!m_OutputPackedFileSystems[fileSystem].WriteFile(fullNameWithExtension, bytes)) return false;
                }
            }

            if (OutputFullSelected)
            {
                var fullNameWithCrc32AndExtension = variant != null
                ...
                var fullPath = Utility.Path.GetRegularPath(Path.Combine(outputFullPath, fullNameWithCrc32AndExtension));
                ...
                {
                    var compressedBytes = Utility.Compression.Compress(bytes);
        ...
        {
            var buildOptions = BuildAssetBundleOptions.DeterministicAssetBundle;

            if (ForceRebuildAssetBundleSelected) buildOptions |= BuildAssetBundleOptions.ForceRebuildAssetBundle;
            ...
            out ResourceData[] assetBundleResourceDatas, out ResourceData[] binaryResourceDatas)
        {
            ...
            foreach (var asset in assets)
            {
                ...
                var assetBytes = File.ReadAllBytes(assetFileFullName);
                var assetHashCode = Utility.Verifier.GetCrc32(assetBytes);

                var dependencyAssetNames = new List<string>();
                var dependencyData = m_ResourceAnalyzerController.GetDependencyData(assetName);
                var dependencyAssets = dependencyData.GetDependencyAssets();
                foreach (var dependencyAsset in dependencyAssets) dependencyAssetNames.Add(dependencyAsset.Name);
            ...
            var assetBundleBuildDataList = new List<AssetBundleBuild>();
            var assetBundleResourceDataList = new List<ResourceData>();
            var binaryResourceDataList = new List<ResourceData>();
            foreach (var resourceData in m_ResourceDatas.Values)
            {
        ...
        private static string GetExtension(ResourceData data)
        {
            ...
            {
                var assetName = data.GetAssetNames()[0];
                var position = assetName.LastIndexOf('.');
                if (position >= 0) return assetName.Substring(position + 1);
            }
