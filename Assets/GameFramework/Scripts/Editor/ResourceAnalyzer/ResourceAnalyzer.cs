...
namespace UnityGameFramework.Editor.ResourceTools
{
    ...
    internal sealed class ResourceAnalyzer : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            {
                GUILayout.Space(5f);
                var toolbarIndex = GUILayout.Toolbar(m_ToolbarIndex,
                    new[]
                    {
        ...
        {
            var window = GetWindow<ResourceAnalyzer>("Resource Analyzer", true);
            window.minSize = new Vector2(800f, 600f);
        }

        private void DrawAnalyzeButton()
        {
            if (!m_Analyzed) EditorGUILayout.HelpBox("Please analyze first.", MessageType.Info);
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.BeginVertical(GUILayout.Width(position.width * 0.4f));
                {
                    ...
                    if (string.IsNullOrEmpty(m_AssetsFilter))
                        ...
                        title = Utility.Text.Format("Assets In Resources ({0}/{1})",
                            m_CachedAssetNames.Length.ToString(), m_AssetCount.ToString());
                    ...
                    EditorGUILayout.BeginVertical("box");
                    {
                        ...
                        EditorGUILayout.BeginHorizontal();
                        {
                            var assetsOrder = (AssetsOrder)EditorGUILayout.EnumPopup("Order by", m_AssetsOrder);
                        ...
                        EditorGUILayout.BeginHorizontal();
                        {
                            var assetsFilter = EditorGUILayout.TextField("Assets Filter", m_AssetsFilter);
                ...
                EditorGUILayout.BeginVertical(GUILayout.Width(position.width * 0.6f - 14f));
                {
                    ...
                    EditorGUILayout.LabelField(
                        Utility.Text.Format("Dependency Resources ({0})",
                            m_SelectedDependencyData.DependencyResourceCount.ToString()), EditorStyles.boldLabel);
                    EditorGUILayout.BeginVertical("box", GUILayout.Height(position.height * 0.2f));
                    {
                        ...
                        {
                            var dependencyResources = m_SelectedDependencyData.GetDependencyResources();
                    ...
                    EditorGUILayout.EndVertical();
                    EditorGUILayout.LabelField(
                        Utility.Text.Format("Dependency Assets ({0})",
                            m_SelectedDependencyData.DependencyAssetCount.ToString()), EditorStyles.boldLabel);
                    EditorGUILayout.BeginVertical("box", GUILayout.Height(position.height * 0.3f));
                    {
                        ...
                        {
                            var dependencyAssets = m_SelectedDependencyData.GetDependencyAssets();
                    ...
                    EditorGUILayout.LabelField(
                        Utility.Text.Format("Scattered Dependency Assets ({0})",
                            m_SelectedDependencyData.ScatteredDependencyAssetCount.ToString()), EditorStyles.boldLabel);
                    EditorGUILayout.BeginVertical("box", GUILayout.Height(position.height * 0.5f - 116f));
                    {
                        ...
                        {
                            var scatteredDependencyAssetNames =
                                m_SelectedDependencyData.GetScatteredDependencyAssetNames();
                            foreach (var scatteredDependencyAssetName in scatteredDependencyAssetNames)
                            {
                                EditorGUILayout.BeginHorizontal();
                                {
                                    var count = m_Controller.GetHostAssets(scatteredDependencyAssetName).Length;
                                    ...
                                    GUILayout.Label(count > 1
                                        ? Utility.Text.Format("{0} ({1})", scatteredDependencyAssetName,
                                            count.ToString())
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.BeginVertical(GUILayout.Width(position.width * 0.4f));
                {
                    ...
                    if (string.IsNullOrEmpty(m_ScatteredAssetsFilter))
                        ...
                        title = Utility.Text.Format("Scattered Assets ({0}/{1})",
                            m_CachedScatteredAssetNames.Length.ToString(), m_ScatteredAssetCount.ToString());
                    EditorGUILayout.LabelField(title, EditorStyles.boldLabel);
                    EditorGUILayout.BeginVertical("box", GUILayout.Height(position.height - 132f));
                    {
                        ...
                        {
                            var selectedIndex = GUILayout.SelectionGrid(m_SelectedScatteredAssetIndex,
                ...
                EditorGUILayout.BeginVertical(GUILayout.Width(position.width * 0.6f - 14f));
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(
                        Utility.Text.Format("Host Assets ({0})", m_SelectedHostAssets.Length.ToString()),
                    ...
                    EditorGUILayout.BeginVertical("box", GUILayout.Height(position.height - 68f));
                    {
                        ...
                        {
                            foreach (var hostAsset in m_SelectedHostAssets)
                            {
                                EditorGUILayout.BeginHorizontal();
                                {
                                    if (GUILayout.Button("GO", GUILayout.Width(30f)))
        ...
        private void DrawCircularDependencyViewer()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.BeginVertical();
                {
                    GUILayout.Space(5f);
                    EditorGUILayout.LabelField(
                        Utility.Text.Format("Circular Dependency ({0})", m_CircularDependencyCount.ToString()),
                        EditorStyles.boldLabel);
                    m_CircularDependencyScroll = EditorGUILayout.BeginScrollView(m_CircularDependencyScroll);
                    {
                        var count = 0;
                        foreach (var circularDependencyData in m_CachedCircularDependencyDatas)
                        {
                            GUILayout.Label(
                                Utility.Text.Format("{0}) {1}", (++count).ToString(),
                                    circularDependencyData[circularDependencyData.Length - 1]), EditorStyles.boldLabel);
                            EditorGUILayout.BeginVertical("box");
                            {
                                foreach (var circularDependency in circularDependencyData)
                                {
                                    EditorGUILayout.BeginHorizontal();
                                    {
        ...
        private void OnAnalyzingAsset(int index, int count)
        {
            EditorUtility.DisplayProgressBar("Analyzing Assets",
                Utility.Text.Format("Analyzing assets, {0}/{1} analyzed.", index.ToString(), count.ToString()),
                (float)index / count);
        }
