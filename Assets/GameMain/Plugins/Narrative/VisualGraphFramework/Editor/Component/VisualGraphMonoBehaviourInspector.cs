...
namespace VisualGraphEditor
{
    ...
    public class VisualGraphMonoBehaviourInspector : Editor
    {
        ...
        public VisualElement CreateObjectPropertyField<Ty>(AbstractBlackboardProperty<Ty> property) where Ty : Object
        {
            var propertyField = new ObjectField(property.Name);
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            var method = target.GetType().BaseType
                .GetMethod("UpdateProperties", BindingFlags.Public | BindingFlags.Instance);
            ...
            // Because everything in Components is a MonoBehaviour we can get the base type
            // If they base type is a generic of type VisualGraphMonoBehaviour<> then we can try and
            var BlackboardPropertyInfo = target.GetType().BaseType.GetField("BlackboardProperties");
            var BlackboardProperties = BlackboardPropertyInfo.GetValue(target) as List<AbstractBlackboardProperty>;

            var blackboardLabel = new Label($"Blackboard Properties: {BlackboardProperties.Count}")
                { name = "blackboardLabel" };
            rootElement.Add(blackboardLabel);

            foreach (var property in BlackboardProperties)
            {
                var blackboardProperty = new VisualElement { name = "blackboardProperty" };
                blackboardProperty.style.flexDirection = FlexDirection.Row;

                var overwriteField = new Toggle();
                ...
                switch (property)
                {
                    ...
                    case ColliderBlackboardProperty prop:
                        fieldElement = CreateObjectPropertyField(prop);
                        break;
